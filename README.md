...
一个面向 ChatGPT/Codex 类智能体的开源黄金研究 Skill。它不依赖“某个指标永远有效”的假设，而是把黄金拆成机会成本、美元与流动性、财政信用、避险与官方需求、跨资产与仓位五条因果链，再用多周期技术面处理趋势确认、入场位置与失效条件，分别形成短期、中期和长期判断。
...
- 分析周线、日线及必要时的小时级价格结构、趋势、动量、波动率、量价与持仓变化，识别支撑阻力区间和技术失效点。
- 输出基准、牛市和熊市情景，以及可观测的确认条件和失效条件。
- 默认展示“数据观察—传导机制—技术确认—结论—失效条件”的完整分析摘要，不再只输出看多/看空结果或评分表。
...
| 跨资产、仓位与技术面 | 金银比、ETF/CFTC、A 股黄金股、价格结构、趋势与动量 | 短期/中期 | 用于择时、拥挤度、关键位置与前瞻确认 |
...
完整解释数据证据、宏观传导和技术面确认，判断 A 股黄金股是否领先，
并给出基准、牛市、熊市情景和失效条件。
```
...
解释从数据到结论的因果路径和技术面确认，判断 A 股黄金股是否构成前瞻信号，
并给出验证条件和失效条件。
```
